  ...
  */
  ...
  setOptions(options) {
    if (options) {
      ...
      util.selectiveExtend(['moment', 'locale', 'locales', 'id', 'title', 'rtl', 'snap', 'mode', 'height', 'core'], this.options, options);
    }
  }
  ...
  _create() {
    ...
    indicator.className = 'vis-customTime-indicator';
    ...
    if (this.options.mode === 'vertical') {
      ...
      drag.className = 'vis-drag-handle';
    }
  ...
  _onDragStart(event) {
    ...
    if (this.options.core.range.isRolling()) {
      this.options.core.range.stopRolling();
    }
  ...
  _onDrag(event) {
    ...
    if (this.options.mode === 'vertical') {
      ...
      delta = this.options.rtl ? (-1) * event.deltaX : event.deltaX;
    }


    const x = this.body.util.toScreen(this.eventParams.customTime) + delta;
