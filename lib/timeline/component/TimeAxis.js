 ...
 */
  ...
  redraw() {
    ...
    const parent = (options.orientation.axis === 'top') ? this.body.dom.top : this.body.dom.bottom;
    ...
    while (step.hasNext() && count < MAX) {
      ...
      if (this.options.showMinorLabels && showMinorGrid) {
        ...
        if (this.options.mode === 'vertical') {
          label.style.height = `${width}px`; // set width to prevent overflow
  ...
  _repaintMinorLine(left, width, orientation, className) {
    ...
    if (this.options.mode === 'vertical') {
      line.style.width = `${props.minorLineHeight}px`;
      line.style.height = `${width}px`;

      let x = props.majorLabelHeight;
      let y = left - props.minorLineWidth / 2;
  ...
  _repaintMajorLine(left, width, orientation, className) {
    ...
    if (this.options.mode === 'vertical') {
      ...
      let x = 0;
      let y = left - props.majorLineWidth / 2;
