 ...
 */
  ...
  setOptions(options) {
    if (options) {
      ...
      util.selectiveExtend(['rtl', 'showCurrentTime', 'alignCurrentTime', 'moment', 'locale', 'locales', 'mode'], this.options, options);
    }
  }
  ...
  redraw() {
    if (this.options.showCurrentTime) {
      ...
      if (this.options.rtl) {
        this.bar.style.transform = (this.options.mode === 'vertical' ? 'translateY' : 'translateX') + `(${x * -1}px)`;
      }
      else {
        this.bar.style.transform = (this.options.mode === 'vertical' ? 'translateY' : 'translateX') + `(${x}px)`;
      }
