 ...
 */
  ...
  _create() {
    ...
    const backgroundGroup = new BackgroundGroup(BACKGROUND, null, this, this.options);
  ...
  setOptions(options) {
    if (options) {
      ...
      const fields = [
        ...
        'hide', 'snap', 'groupOrderSwap', 'showTooltips', 'tooltip', 'tooltipOnItemUpdateTime', 'groupHeightMode', 'onTimeout', 'mode'
      ];
 ...
 */
  ...
  _onDragStartAddItem(event) {
    ...
    const props = {
      item: newItem,
      initialX: event.center.y, // TODO: vertical mode
      data: newItem.data
    };
  ...
  _onDrag(event) {
    ...
    if (this.touchParams.itemProps) {
      ...
      const me = this;
      const snap = this.options.snap || null;
      const domRootOffsetLeft = this.body.dom.root.offsetTop; // TODO: vertical mode
      ...
      this.touchParams.itemProps.forEach(props => {
        const current = me.body.util.toTime(event.center.y - xOffset); // TODO: vertical mode
        const initial = me.body.util.toTime(props.initialX - xOffset); // TODO: vertical mode
        let offset;
        let initialStart;
        let initialEnd;
        let start;
