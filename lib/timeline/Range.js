 ...
 */
  ...
  /**
   * Determine if the range is rolling to follow custom time.
   *
   * @returns {boolean}
   */
  isRolling() {
  ...
  /**
   * Determine if the range is following any time marker
   *
   * @returns {boolean}
   */
  isFollowing() {
  ...
  startRolling() {
    ...
    function update() {
      ...
      if (me.following) {
        const customTime = me.parent.customTimes.find((customTime) => customTime.options.id === me.following);
        t = util.convert(customTime.customTime, 'Date').valueOf();
      }
      else {
        t = util.convert(new Date(), 'Date').valueOf();
      }
  ...
  /**
   * Start to follow (rolling option) a custom time that may be moving.
   *
   * @param {string} id
   */
  followCustomTime(id) {
  ...
  /**
   * Set a reference to the parent component (Timeline) to follow custom time
   *
  ...
  /**
   * Switch rolling back to current time.
   */
  stopFollowingCustomTime() {
    this.following = false;
  }
