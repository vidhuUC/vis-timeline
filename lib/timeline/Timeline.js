 ...
 */
  ...
  /**
   * Toggle following (rolling) on custom time
   *
   * @param {string} id
   */
  toggleFollowCustomTime(id) {
    if (this.range.following) {
      this.range.stopFollowingCustomTime();
    }
    else {
      this.range.followCustomTime(id);
    }
  }
}
