...
class Core {
    ...
    _redraw() {
        ...
        let rootOffsetWidth = 0;

        dom.root.style.width = util.option.asSize(options.width, '');

        rootOffsetWidth = dom.root.offsetWidth;
        ...
        if (options.mode === 'vertical') {
            props.center.height = util.option.asSize(options.height, '');
            props.top.height = util.option.asSize(options.height, '');
        }
        else {
    ...
    _setDOM() {
        if (this.options.mode === 'vertical') {
            ...
            props.leftContainer.height = options.height;
            props.rightContainer.height = options.height;

            props.center.height = options.height.replace('px', '');
            props.centerContainer.height = options.height.replace('px', '');
            props.top.height = options.height.replace('px', '');

            // resize the panels

            dom.background.style.height = options.height;
            dom.backgroundVertical.style.height = options.height;
            dom.backgroundHorizontal.style.height = options.height;
            dom.centerContainer.style.height = options.height;
            dom.top.style.height = options.height;
            dom.bottom.style.height = options.height;

            dom.background.style.width = "100%";
            dom.backgroundVertical.style.width = "100%";
            dom.backgroundHorizontal.style.width = "calc(100% - 90px)";
            dom.centerContainer.style.width = "calc(100% - 90px)";
            dom.leftContainer.style.width = "100%";
            dom.rightContainer.style.width = "100%";
            dom.top.style.width = "90px";
            dom.bottom.style.width = "100%";

            dom.center.style.height = '100%';

            // reposition the panels
            dom.background.style.left = 0;
            dom.background.style.top = 0;
            dom.backgroundHorizontal.style.top = 0;
            dom.backgroundHorizontal.style.left = '90px';
            dom.leftContainer.style.top = `${props.top.height}px`;
            dom.leftContainer.style.left = 0;
            dom.top.style.top = 0;
            dom.top.style.left = 0;
            dom.backgroundVertical.style.top = 0;
            dom.backgroundVertical.style.left = 0;
            dom.backgroundHorizontal.style.top = 0;
            dom.backgroundHorizontal.style.left = `90px`;
            dom.centerContainer.style.top = 0;
            dom.centerContainer.style.left = `90px`;
            dom.rightContainer.style.top = 0;
            dom.rightContainer.style.left = 0;
            dom.bottom.style.left = 0;
            dom.bottom.style.top = options.height;
        }
        else {
            const props = this.props;
            const dom = this.dom;
    ...
    _toTime(x) {
        if (this.options.mode === 'vertical') {
            return DateUtil.toTime(this, x, parseInt(this.props.center.height));
        }
    ...
    _toScreen(time) {
        if (this.options.mode === 'vertical') {
            return DateUtil.toScreen(this, time, parseInt(this.props.center.height));
        }
